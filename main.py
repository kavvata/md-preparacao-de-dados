...
moda_forma_benigna = st.mode(
    [
        int(shape)
        for shape in df_mamo.loc[
            (df_mamo["severity"] == 0) & (df_mamo["shape"] != "?"), "shape"
        ]
    ]
)

moda_forma_maligna = st.mode(
    [
        int(shape)
        for shape in df_mamo.loc[
...
moda_contorno_benigna = st.mode(
    [
        int(margin)
        for margin in df_mamo.loc[
            (df_mamo["severity"] == 0) & (df_mamo["margin"] != "?"), "margin"
        ]
    ]
)

moda_contorno_maligna = st.mode(
    [
        int(margin)
        for margin in df_mamo.loc[
...
moda_densidade_benigna = st.mode(
    [
        int(density)
        for density in df_mamo.loc[
            (df_mamo["severity"] == 0) & (df_mamo["density"] != "?"), "density"
        ]
    ]
)
moda_densidade_maligna = st.mode(
    [
        int(density)
        for density in df_mamo.loc[
